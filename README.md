...
The floating “Ask about my profile” chat widget now talks to a **Vercel serverless route** at `/chat` which calls OpenAI directly using your key.

1. In your Vercel project → **Settings → Environment Variables**, add:
   - **`OPENAI_API_KEY`** – your OpenAI API key
   - Optional: **`OPENAI_MODEL`** – e.g. `gpt-4o-mini`

2. Redeploy the portfolio. The widget will call `/chat` on the same origin, and the route in `app/chat/route.ts` will handle the conversation using OpenAI.
